window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dshdb-crumb{display:flex;align-items:center;gap:2px;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;flex:1 1 100%;min-width:0;font-size:12px;color:var(--dsw-alias-label-secondary,#9a9a9a)}",
      ...
      ".dshdb-mvchip{display:inline-flex;align-items:center;gap:5px;font-size:12px;color:var(--dsw-alias-state-business-primary,#4176e6);max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      ".dshdb-row-active{background:var(--dsw-alias-interactive-bg-hover,#80808024);box-shadow:inset 2px 0 0 var(--dsw-alias-state-business-primary,#4176e6)}",
    ...
    var MV = h("svg", { width: 12, height: 12, viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.6, "aria-hidden": true },
      h("path", { d: "M2 8h8m0 0-3-3m3 3-3 3M12.5 3.5h1.5v9h-1.5" }));
    ...
    function Browser(props) {
      ...
      var mv0 = useState(null);
      var mv = mv0[0], setMv = mv0[1];
      ...
      var createFolder = function () {
        var nm = (window.prompt("\u65b0\u5efa\u6587\u4ef6\u5939\u540d\u79f0\uff08\u5f53\u524d\u76ee\u5f55\uff09") || "").trim();
        if (!nm) return;
        var qs = new URLSearchParams({ dir: cwd, name: nm });
        fetch("/disk-browser/api/mkdir?" + qs.toString(), { method: "POST" })
          .then(function (r) { return r.json().then(function (j) { return { status: r.status, j: j }; }); })
          .then(function (o) {
            if (!o.j.ok) {
              setErr(o.status === 409 ? "\u6587\u4ef6\u5939\u5df2\u5b58\u5728:" + nm : (o.j.error || "mkdir-failed"));
              return;
            }
            setErr("");
            load(cwd);
          })
          .catch(function () { setErr("network"); });
      };

      var doMove = function () {
        if (!mv) return;
        var qs = new URLSearchParams({ from: fullPath(mv.dir, mv.name), toDir: cwd });
        fetch("/disk-browser/api/move?" + qs.toString(), { method: "POST" })
          .then(function (r) { return r.json().then(function (j) { return { status: r.status, j: j }; }); })
          .then(function (o) {
            if (!o.j.ok) {
              setErr(o.status === 409 ? "\u76ee\u6807\u5df2\u5b58\u5728\u540c\u540d\u9879:" + mv.name
                : o.j.error === "invalid-target" ? "\u4e0d\u80fd\u79fb\u52a8\u5230\u81ea\u8eab\u5b50\u76ee\u5f55\u5185"
                : (o.j.error || "move-failed"));
              return;
            }
            setMv(null);
            setErr("");
      ...
      return h("div", { className: "dshdb-layer", onClick: props.onClose },
        h("div", { className: "dshdb-dlg", onClick: function (e) { e.stopPropagation(); } },
          ...
          h("div", { className: "dshdb-toolbar" },
            ...
            mv ? h("span", { className: "dshdb-mvchip" }, MV, "\u79fb\u52a8\u4e2d:" + mv.name) : null,
            mv ? h("button", { className: "dshdb-btn", onClick: doMove }, "\u79fb\u52a8\u5230\u6b64\u5904") : null,
            mv ? h("button", { className: "dshdb-btn", onClick: function () { setMv(null); setErr(""); } }, "\u53d6\u6d88") : null,
            mv ? null : h("button", { className: "dshdb-btn", disabled: !!upStatus, onClick: function () { if (fileRef.current) fileRef.current.click(); } }, UL, "\u4e0a\u4f20"),
            h("input", {
              ref: fileRef, type: "file", multiple: true, style: { display: "none" },
              onChange: function (e) { uploadFiles(e.target.files); e.target.value = ""; }
            }),
            mv ? null : h("button", { className: "dshdb-btn", onClick: createFolder }, FOLDER, "\u65b0\u5efa\u6587\u4ef6\u5939"),
          ...
          h("div", { className: "dshdb-body" },
            h("div", {
              ...
              data ? data.entries.map(function (e) {
                return h("button", {
                  key: e.name,
                  className: "dshdb-row" + (e.dir ? " dshdb-dir" : "") + (e.hidden ? " dshdb-hidden" : "") + (mv && mv.name === e.name && mv.dir === cwd ? " dshdb-row-active" : ""),
                  onClick: function () {
                    if (mv) { if (e.dir) load(fullPath(cwd, e.name)); return; }
                    if (e.dir) load(fullPath(cwd, e.name)); else openFile(e.name);
                  }
                },
                  e.dir ? FOLDER : FILE,
                  ...
                  h("span", {
                    role: "button", className: "dshdb-dl", title: "\u79fb\u52a8",
                    onClick: function (ev) { ev.stopPropagation(); setMv({ name: e.name, dir: cwd }); setErr(""); setPv(null); }
                  }, MV),
                  e.dir ? null : h("span", {
          ...
          h("div", { className: "dshdb-status" },
            upStatus || (mv ? "\u79fb\u52a8\u4e2d:" + mv.name + " \u2014 \u5bfc\u822a\u5230\u76ee\u6807\u6587\u4ef6\u5939\u540e\u70b9\u300c\u79fb\u52a8\u5230\u6b64\u5904\u300d\uff0c\u6216\u300c\u53d6\u6d88\u300d"
              : (data ? cwd + " \u00b7 " + data.entries.length + " \u9879" : "")))));
    }
