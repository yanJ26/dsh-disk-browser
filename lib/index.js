import { readdir, stat, open, unlink, mkdir, rename } from 'node:fs/promises'
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const toDir = resolve(q.get('toDir') || '')
        const nm = basename(from)
        if (!nm || nm === '.' || nm === '..') {
          return writeJson(res, { ok: false, error: 'bad-name' }, 400)
        }
        const src = await stat(from).catch(() => null)
        if (!src) return writeJson(res, { ok: false, error: 'not-found' }, 400)
        const ds = await stat(toDir).catch(() => null)
        if (!ds || !ds.isDirectory()) return writeJson(res, { ok: false, error: 'bad-dir' }, 400)
        const dst = join(toDir, nm)
        if (dst === from) return writeJson(res, { ok: true, path: dst, noop: true })
        if (src.isDirectory() && (toDir === from || toDir.startsWith(from + '/'))) {
          return writeJson(res, { ok: false, error: 'invalid-target' }, 400)
        }
        const ex = await stat(dst).catch(() => null)
        if (ex) return writeJson(res, { ok: false, error: 'exists' }, 409)
